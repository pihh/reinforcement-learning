            ...
            while not done:
                ...
                if time_count == update_freq or done:
                    ...
                    if done:  # done and print information
                        Worker.global_moving_average_reward = ep_reward * 0.05 + Worker.global_moving_average_reward *0.95
                        # Worker.global_moving_average_reward = \
                        # record(Worker.global_episode, ep_reward, self.worker_idx,
                        #         Worker.global_moving_average_reward, self.result_queue,
                        #         self.ep_loss, ep_steps)
                        # We must use a lock to save our model and to print to prevent data races.
                        print('episode {} done with score: {}'.format(Worker.global_episode,ep_reward))

                        # We must use a lock to save our model and to print to prevent data races.
                        if Worker.global_moving_average_reward > Worker.best_score:
                            with Worker.save_lock:
                                print("Saving best model to *pass*, ""episode score: {}".format( Worker.global_moving_average_reward))
                                # self.global_model.save_weights(
                                #     os.path.join(self.save_dir,
                                #                 'model_{}.h5'.format(self.game_name))
                                # )
                                Worker.best_score = Worker.global_moving_average_reward
                ...
                total_step += 1
            self.result_queue.put(Worker.global_moving_average_reward)
...
class MasterAgent():
    def __init__(self):
        ...
        self.opt = tf.compat.v1.train.AdamOptimizer(learning_rate=0.001, use_locking=True)
    ...
    def train(self):
        ...
        workers = [Worker(self.state_size,
                        ...
                        ) for i in range(4)] #multiprocessing.cpu_count())]
