...
class ActorNetwork():
    ...
    def act_batch(self,states):
        ...
        if self.action_space_mode == "discrete":
            ...
            predictions_list = self.model.predict(states)
            mu_list = [p[0] for p in predictions_list]
            sigma_list = [p[1] for p in predictions_list]
            actions_list = [np.random.normal(mu_list[i], sigma_list[i] ,self.n_actions) for i in range(len(predictions_list))]
            for i,action in enumerate(actions_list):
                actions_list[i] = np.clip(action, -self.action_bound, self.action_bound)
            actions_onehot_list = actions_list.copy()
...
class Worker(Process):
    def __init__(self, worker_idx, child_connection, child_environment):
        super(Worker, self).__init__()
        
        #  Create env here
        self.worker_idx = worker_idx
        self.child_connection = child_connection
        self.env = child_environment(describe=False)
    ...
    def learn(self,n_workers=cpu_count()):
        ...
        while episode < episodes:
            ...
            for worker_id, parent_connection in enumerate(parent_connections):
                ...
                if buffers[worker_id].size >= np.random.randint(self.batch_size,high=self.batch_size*2, size=1)[0]:
                ...
                if done:
                    if(episode < episodes):
                        ...
                        if average < 200:
                            #print('episode ',episode,'worker',worker_id, 'score',score[worker_id], 'avg', round(average,2), 'replay count', replay_count)
                            if episode % 10 == 0:
